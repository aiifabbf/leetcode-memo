"""
把一个数组分成两个非空subarray（不一定要连续），使得两个subarray的平均值相等。如果可以做到，就返回true，也有可能做不到，那就返回false。

难点在于是subarray而不是substring，如果是substring就很好办……

有一个定理是，如果这个array里存在一个subarray使得这个subarray的平均值和剩下的所有的值的平均值相等，那么这个subarray的平均值一定就是整个array的平均值。

.. note::

    可以简单证明一下，假设array是 :math:`\{a_1, a_2, ..., a_n\}` ，经过重排之后，array变成 :math:`\{a'_1, a'_2, ..., a'_n\}` ，其中前 :math:`j` 个元素就是这个满足条件的subarray。因此有
    ...
    .. math::
        ...
        \begin{aligned}
            j \sum_{i = 1}^n a'_i &= n \underbrace{\sum_{i = 1}^j a'_i}^{subarray之和} \\
